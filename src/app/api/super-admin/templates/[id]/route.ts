/**
 * Super Admin Template by ID API
 * GET: Fetch a single template by ID
 * PUT: Update an existing system template
 */
...
export async function PUT(
  ...
  try {
    ...
    // Check if user can edit this template (only system templates)
    if (template.scope !== 'system') {
      return NextResponse.json(
        { error: 'Super admins can only edit system templates' },
        { status: 403 },
      );
    }
