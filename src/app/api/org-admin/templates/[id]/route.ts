/**
 * Org Admin Template by ID API
 * GET: Fetch a single template by ID
 * PUT: Update an existing organization template
 */
...
export async function GET(
  ...
  try {
    ...
    // Org admin can access: system templates or templates from their organization
...
export async function PUT(
  ...
  try {
    ...
    // Check if user can edit this template (organization templates from their org)
    if (template.scope !== 'organization' || template.organizationId !== user.organizationId) {
      return NextResponse.json(
        { error: 'You can only edit templates from your organization' },
        { status: 403 },
      );
    }
