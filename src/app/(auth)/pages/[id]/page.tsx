 ...
 * Design matches the public share page for consistent experience
 */

'use client';

import { Heart, Music, Send, StickyNote } from 'lucide-react';
...
export default function StoryViewerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchStoryPage = async () => {
    try {
      ...
      const response = await fetch(`/api/pages/${resolvedParams.id}?patientView=true`, {
        headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const reflectionEntries = Object.entries(reflectionAnswers).filter(([, answer]) => answer.trim());
      if (reflectionEntries.length > 0) {
        const responses = reflectionEntries.map(([questionId, answer]) => ({
          questionId,
          pageId: resolvedParams.id,
          responseText: answer,
        }));

        const refResponse = await fetch('/api/responses/reflection', {
          method: 'POST',
          headers: {
      ...
      const surveyEntries = Object.entries(surveyAnswers).filter(([, answer]) => answer !== '' && answer !== undefined);
      if (surveyEntries.length > 0) {
        const responses = surveyEntries.map(([questionId, answer]) => ({
          questionId,
          pageId: resolvedParams.id,
          responseValue: typeof answer === 'number' ? String(answer) : answer,
          responseNumeric: typeof answer === 'number' ? answer : (!isNaN(Number(answer)) ? Number(answer) : null),
        }));

        const surveyResponse = await fetch('/api/responses/survey', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${idToken}`,
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-white via-purple-50/30 to-white px-4">
        <div className="max-w-md text-center">
          ...
          <h1 className="mb-3 text-2xl font-bold tracking-tight text-gray-800">Error Loading Story</h1>
          ...
          <button
            ...
            className="mt-6 rounded-full bg-gray-100 px-6 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200"
          >
            Go Back
          </button>
        </div>
  ...
  const hasQuestions = pageData.reflectionQuestions.length > 0 || pageData.surveyQuestions.length > 0;
            ...
            <div key={block.id || index}>
              ...
              {block.blockType === 'quote' && block.textContent && (
                <blockquote className="relative my-12 py-8 sm:my-16">
                  ...
                  <div className="pl-8 sm:pl-12">
                    ...
                    {(block.settings?.attribution || block.settings?.speakerName) && (
              ...
              {block.blockType === 'note' && block.textContent && (
                <div className="my-8 sm:my-12">
                  <div className="rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-yellow-50 px-6 py-5">
                      ...
                      <div key={q.id}>
                        ...
                        {q.questionType === 'emotion' && (
                          <div className="flex gap-2 sm:gap-4">
                            {[
                              { emoji: '\u{1F622}', label: 'Very Sad', value: 1 },
                              { emoji: '\u{1F615}', label: 'Sad', value: 2 },
                              { emoji: '\u{1F610}', label: 'Neutral', value: 3 },
                              { emoji: '\u{1F642}', label: 'Happy', value: 4 },
                              { emoji: '\u{1F604}', label: 'Very Happy', value: 5 },
                            ].map(emotion => (
                              <button
                                key={emotion.value}
                                type="button"
                        ...
                        {q.questionType === 'multiple_choice' && q.options && (
                          <div className="space-y-3">
                            {(typeof q.options === 'string' ? JSON.parse(q.options) : q.options).map((option: string, optIndex: number) => (
                              <button
                                key={optIndex}
                                type="button"
