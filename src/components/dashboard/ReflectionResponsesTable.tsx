...
type ReflectionResponsesTableProps = {
  responses: ReflectionResponse[];
  loading?: boolean;
};

export function ReflectionResponsesTable({ responses, loading = false }: ReflectionResponsesTableProps) {
  if (loading) {
    return (
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-gray-900">Recent Reflection Responses</h2>
          </div>
        </CardHeader>
        <CardBody>
      ...
      <Card>
        ...
        <CardBody>
          <div className="py-8 text-center">
            ...
            <p className="text-sm text-gray-500">No reflection responses yet</p>
          </div>
        </CardBody>
      </Card>
    );
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {responses.map(response => (
                <tr key={response.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <p className="max-w-md text-sm text-gray-600 italic">
                      "
                      {response.response}
                      "
                    </p>
